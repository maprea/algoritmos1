...
Ambas variables de tipo _Double_ pueden compararse con el operador _==_ para determinar si son la misma referencia, caso contrario el compilador detectaría que son de distinto tipo y no podrían compararse así. En este caso, la comparación de identidad es falsa porque _a_ y _b_ son dos instancias distintas de _Double_. La comparación de igualdad sí es verdadera porque en ese caso el método _equals_ de _Double_ (sobreescribe al de _Object_) compara el valor que almacena esa referencia.

> **Ojo con _Integer_**: si hiciéramos el mismo ejemplo con `Integer a = 10; Integer b = Integer.valueOf(10);` la comparación con _==_ daría **true**. Esto se debe a que Java mantiene una _caché_ de instancias de _Integer_ para los valores entre -128 y 127, por lo que ambas variables terminan apuntando a la misma instancia. Con valores fuera de ese rango (por ejemplo 1000) volvería a dar **false**. Esta caché no existe para _Double_, y es un buen ejemplo de por qué **no debemos apoyarnos en _==_ para comparar valores de objetos**.
